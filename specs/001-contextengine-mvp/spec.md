...
- **FR-021a**: Beyond the per-answer panel, the system MUST emit **operational telemetry** for every agent step — structured lifecycle records (start / completed / failed) carrying a correlation set that ties a step to its run, its trace, and the deployed build; and aggregate metrics for step duration, outcome, degradation, tool and model calls, run cost, and budget exhaustion. A single run's panel cannot show whether a stage is degrading across the population, and a panel nobody has opened reports nothing. Telemetry MUST carry **metadata only** — never prompt or response bodies, retrieved content, memories, or the member's question (FR-024's single-writer rule) — and metric dimensions MUST come from a closed, low-cardinality vocabulary that excludes tenant, member, and resource identifiers, so a dashboard cannot become a cross-tenant activity oracle. Emission MUST degrade cleanly: with no telemetry backend configured, the system still serves requests.
...
- **FR-028a**: Every agent run MUST additionally be bounded in **time and work**, not only in money: a wall-clock deadline on both interactive and long-horizon runs, and a maximum number of steps on a long-horizon run, both admin-configurable and evaluated at a resumable boundary. Time spent awaiting a human approval MUST NOT count against the deadline — a gate held open for a day is the feature working, not a run to be killed. On breach the run MUST end in a recorded failure with the reason attributable and spend already incurred settled, never silently retried. A per-step timeout bounds a step, not a run; cost caps bound money, not a stalled call, so neither alone prevents a run from consuming a worker indefinitely.
...
- **FR-030a**: The evaluation set MUST **grow from observed failures**: every agent-behavior defect found in staging or production — a wrong tool chosen, a hallucinated claim, an unnecessary clarification, a retrieval miss — MUST be added to it as a permanent case, reproducing the failure from its recorded trace, before the fix is considered complete. A defect that is fixed but not captured can return silently, and a suite that only ever tests what was imagined at design time measures the designers rather than the system.
