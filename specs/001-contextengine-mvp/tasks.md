...
- [ ] T003 [P] Initialize Python project in `backend-python/pyproject.toml` (Python 3.12) with FastAPI, LangGraph, Mem0, BAML, FastMCP, `e2b` (sandbox-tier client), qdrant-client, `openai` (client pointed at the LLM gateway URL — provider SDKs live in the gateway service, not here), structlog, Langfuse SDK, and `testcontainers` (pytest integration deps). **MarkItDown** and **Crawl4AI/Playwright** are declared as the `convert` / `crawl` **optional-dependency extras** that build the sandbox microVM templates (`deploy/sandbox/templates/`), not the base image (research §24)
...
- [ ] T024c [P] Deploy the **standalone self-hosted sandbox tier** (E2B/Firecracker microVMs; Daytona/gVisor-swappable, E2B-Cloud-optional) per [sandbox-runtime.md](./contracts/sandbox-runtime.md) + `deploy/sandbox/`: fleet control plane + template registry (`tmpl-crawl`/`tmpl-convert`/`tmpl-coderun`) + **default-deny egress proxy** (crawl gets the `web_distill` SSRF allowlist, convert gets none); `SANDBOX_URL`/`SANDBOX_KIND` config; the dedicated tainted KVM/`*.metal` node group (`deploy/eks/terraform/sandbox.tf`) or a DO bare-metal/KVM host running `e2b-dev/infra`; the **only** holder of the fleet + keys. Local dev uses `SANDBOX_KIND=local_docker`/`daytona` (no `/dev/kvm`) (research §24)
...
- [ ] T026a [P] Contract test for the **`Sandbox` port + tier** (egress **default-deny** on `tmpl-convert`; SSRF parity on `tmpl-crawl`; **no** ambient provider key / DB / Qdrant reachability from inside; `timeout_s` kill → `timed_out=true` spends only used compute; **exactly one** `sandbox_run` row + **one** `billing.deduct` `operation_type=sandbox.*` per run — Python never writes the ledger; `SANDBOX_KIND` swap changes no caller code) in `backend-python/tests/contract/test_sandbox_runtime.py` per [sandbox-runtime.md](./contracts/sandbox-runtime.md) (SC-001, SC-006)
- [ ] T026b Implement the Python **`Sandbox` port + E2B adapter** (`SandboxSpec`/`SandboxResult`; `run`/`run_code`; stage `files_in` from S3 **clearance-scoped**; hard `vcpu`/`mem_mb`/`timeout_s` caps; compute metering `vcpu_seconds`/`egress_bytes` → `billing.deduct` `operation_type=sandbox.*` via the spend emitter of T098; one `sandbox_run` audit row per run; **no** provider key/DB handed in; swappable by `SANDBOX_KIND` — `e2b_selfhost`|`e2b_cloud`|`daytona`|`local_docker`) in `backend-python/src/services/sandbox/client.py` + `backend-python/src/services/sandbox/e2b.py` (research §24, SC-006, SC-001)
...
- [ ] T049 [P] [US1] Implement MarkItDown converter (PDF/DOCX/MD) — running the parse **inside a `tmpl-convert` sandbox** via the `Sandbox` port (T026b), so untrusted-file parsing (a real parser-RCE surface) is isolated in a **network-denied** microVM instead of in-process on the ingest pod; returns Markdown to the pipeline — in `backend-python/src/services/ingestion/markitdown.py` (FR-001, research §24)
...
- [ ] T051 [P] [US1] Implement shared `web_distill(urls, intent)` capability — SSRF-guarded fetch (https-only; reject private/loopback/link-local/reserved IPs after resolving all A/AAAA; `redirect:error`; bounded size+timeout) → Crawl4AI → distill-against-intent — in `backend-python/src/services/ingestion/web_distill.py` (FR-001). The **Crawl4AI headless-browser fetch runs inside a `tmpl-crawl` sandbox** via the `Sandbox` port (T026b), with the SSRF allowlist enforced at the fleet **egress proxy** (defense-in-depth over the in-code guard); the browser lives in the microVM, not the pod (research §24). Reused by the Phase-1 `web_search` action tool (T072a).
