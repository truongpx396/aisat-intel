...
- [ ] T011 [P] Define kernel interfaces in `backend-go/kernel/auth.go`, `bus.go`, `storage.go`, `mailer.go` (email-send port, default Resend adapter), `meter.go`, `notify.go` (notification ports — `Notifier`/`Channel`/`ChannelRegistry`/`Store`/`PreferenceStore`/`TemplateRenderer` + opaque `Recipient`/`Tenant`/`Topic`, per [notification-ports.md](./contracts/notification-ports.md)), `flags.go`, `cache.go`, `actor.go` (consumer-defined, no product imports)
