...
- Sandbox runtime: **self-hosted E2B** (Firecracker microVMs; Daytona/gVisor-swappable, E2B-Cloud-optional — research §24) as a standalone tier — the single chokepoint for isolated code/tool exec (crawl4ai, MarkItDown convert, Phase-2 code-gen); the only holder of the fleet + template registry + egress policy
...
│   ├── sandbox-runtime.md   # Sandbox tier (E2B/Firecracker; Daytona-swappable) + Sandbox port
...
│   │   ├── sandbox/             # thin client to the standalone sandbox tier (E2B/Firecracker; Daytona-swappable): stage-files·run·metered·audited (research §24)
│   │   ├── ingestion/           # pipeline, chunker, captioner, markitdown, web_distill, enrich, tagger, crawl_orchestrator (crawl4ai fetch + markitdown convert run inside sandbox microVMs)
