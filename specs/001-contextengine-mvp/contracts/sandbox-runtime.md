...
| **`tmpl-coderun`** (Phase 2) | **`opensandbox`** | **`runsc`** | The default. Coderun was already gated on a per-job substrate, so this replaces work we would otherwise write. `k8s_pod` / `oneshot` remain supported fallbacks. No metal, no `/dev/kvm`, $0. |
...
- **`SANDBOX_KIND`** — *how sandboxes are orchestrated*: `opensandbox` | `service` | `oneshot` | `k8s_pod` | `docker` | `e2b_selfhost` | `e2b_cloud`, plus `SANDBOX_URL`. **`SANDBOX_KIND` is resolved per template**, so `tmpl-coderun` can default to `opensandbox` while crawl/convert stay on `service` — the same per-template resolution that makes `SANDBOX_RUNTIME` work. This also fixes the **lifecycle** (long-lived pool vs. per job) — see invariant 12.
...
|----------------|---------------|-----------|---------------|------|--------|
| **`opensandbox`** | [OpenSandbox](https://github.com/opensandbox-group/OpenSandbox) (Apache-2.0) — purpose-built agent-sandbox runtime; gVisor/Kata/Firecracker over Docker **or** Kubernetes, the latter building on `kubernetes-sigs/agent-sandbox` | per job | its own control plane | $0 (self-hosted) | **default for `tmpl-coderun`** — adoption gated, [research §24 (i)](../research.md) |
| **`service`** | Long-lived pool reached over HTTP. **Creates nothing at runtime → no socket, ever** | **stateless per job**, recycled on a bound (inv. 12) | compose / any host | **$0** | **DO Phase 1** (crawl + convert) |
...
> **`opensandbox` is the default for `tmpl-coderun`, and its adoption is *gated*.** [OpenSandbox](https://github.com/opensandbox-group/OpenSandbox) (Apache-2.0, ~12.3k stars, active) ships gVisor/Kata/Firecracker across Docker **and** Kubernetes runtimes; the Kubernetes path builds on `kubernetes-sigs/agent-sandbox`, and it documents per-sandbox egress controls and a creation-time `timeout`. Choosing it changes nothing structural: it replaces only the *mechanical* half, since invariants 3, 4, 5, 7, 9 and 12 are domain obligations that stay in this port and the Go control plane whatever executes the container — so callers and contract tests are unchanged.
>
> **Two gates remain open and are decisive** ([research §24 (i)](../research.md)): (1) its **Docker** runtime almost certainly requires a host container-runtime socket, which would breach **invariant 8** on plain compose — the exact hazard `oneshot` exists to avoid; and (2) **`max_runs = 1` / destroy-after-one-job is unconfirmed** — a `timeout` is not the same guarantee, and reuse of an instance that ran model-authored code is the one thing this template cannot tolerate. Until both are verified, **`oneshot` remains the shipping fallback on compose**, and `opensandbox` is the default only where its Kubernetes runtime is available. Egress controls (question 3) appear satisfied by its documented per-sandbox egress feature; RBAC narrowness (question 4) still needs reading.
