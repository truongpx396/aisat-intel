...
- A **chat attachment** is likewise just a Document — uploaded from the composer with `scope='personal'` and `access_level` = the uploader's clearance, then linked to its conversation by `chat_session_attachment (session_id, document_id, created_at)` (FR-043). It is **not** a separate entity or a hidden copy: it appears in the owner's library, is invisible to every other member (personal scope is owner-only, clearance-irrelevant), is metered as an ingestion, and is deleted via `DELETE /documents/{id}` like anything else. Deleting the **conversation** does not delete it — an attachment is content the member supplied, unlike a Mem0 memory, which is derived from the conversation and *is* purged with it (see Chat Session).
...
- `title` TEXT — the conversation label shown in the session list. Written **once**, after the first turn, from the `rewrite` node's normalized query (already computed on the critical path, so a title costs **no extra LLM call and no credits**; a rewritten query is by construction the self-contained form of "what was this chat about"). Falls back to the truncated raw first message if `rewrite` degraded. Member-editable thereafter (`PATCH`), and a member edit is never overwritten by the system.
- `last_message_at` TIMESTAMPTZ — advanced on every completed turn. **This, not `created_at`, orders the session list**: a conversation replied to minutes ago must outrank one opened yesterday and abandoned. Index `(user_id, archived_at, last_message_at DESC)`.
- `archived_at` TIMESTAMPTZ NULL — soft-hide from the default list without destroying history or memory. Distinct from delete (below), which is irreversible.
- **Deletion purges memory, not just the row** (privacy invariant): deleting a session MUST also delete its Mem0 namespace (`mem0_session_id`). Mem0 memories are injected at Node 5 on **every later query**, so a session deleted from the list while its memories survive would keep invisibly shaping future answers — a member who "deleted the conversation" would be wrong about what the system still knows. The purge is part of the same operation and is retried until it succeeds; a failed purge leaves the session row in `deleting` rather than reporting success. Archiving deliberately does **not** purge — that is the difference between the two actions, and the UI must say so.
...
- `agent_policies`: `id`, `workspace_id`, `agent_role` (`user`|`admin`|`automation`|`integration`), `allowed_tools[]` (MCP tool names), `token_budget_day` INT, `max_loop_depth` INT (default 20), **`run_deadline_s` INT (default 1800)**, **`max_steps` INT (default 60)**, `hooks_enabled[]` (`audit`|`langfuse`|`garak`), **`can_write` BOOL (default false)**, **`write_ops[]`** (Phase 1: only `note_update`), **`write_max_level` INT** (upper bound on an agent write's access level, ≤ owner clearance), `created_at`
  - **Run-level budgets ([contracts/agent-graph.md](./contracts/agent-graph.md) §Run-level budgets):** `run_deadline_s` and `max_steps` bound a **durable** run the way the per-node timeouts bound a step — evaluated at each node boundary, with **paused time at a human gate excluded** from the deadline. They sit alongside the existing `token_budget_day` (admission) and per-run `credits_cap` (`agent_run`) so no single counter is the only thing standing between a misbehaving loop and the budget. Interactive runs use a fixed 120s deadline and need no step cap (single forward pass).
  ...
  - **Reusability seam — this row *is* the agent manifest** ([contracts/agent-runtime.md](./contracts/agent-runtime.md)): the runtime reads this record as a self-contained `AgentManifest`, so a new domain is a manifest swap + a thin `DomainPlugin` (tool bodies + `Authorizer` `Policy`), never a graph edit. Phase-2+ manifests extend it **additively** (prompt refs, `models` alias map, `retrieval` binding, `bus` binding, `mcp_servers[]`, `channels[]`) — no new table, no isolation fork. **Config selects a `Policy`, never *is* one:** these fields only *narrow* an agent within the principal's clearance; the row-level pre-filter floor (SC-001) is enforced below the agent and is **profile-invariant** — on the full AISAT profile it lowers to RLS **+** the Qdrant payload filter, on a self-contained single-store profile it lowers to Postgres RLS alone (pgvector). Fewer lowerings is fewer copies to keep in parity, not fewer guarantees.
...
### Audit Record (P + K)
- `agent_audit_log` (P): `id`, `workspace_id`, `user_id`, `agent_role`, `tool_called`, `token_cost`, `result_hash` (tamper-evident result fingerprint), `trace_id`, `created_at`, **`seq` BIGINT + `prev_hash` + `entry_hash` (per-tenant tamper-evident chain link)**. Partitioned by `created_at`.
- `audit_log` (K): generic workspace/member actions — `id`, `workspace_id`, `actor_type`, `actor_id`, `action`, `resource_type`, `resource_id`, `metadata` JSONB, `created_at`, **`seq` BIGINT + `prev_hash` + `entry_hash` (per-tenant tamper-evident chain link)**. Partitioned by `created_at`.
- Rules: append-only; AI tool calls and workspace/member actions both audited (FR-023).
- **Tamper-evidence (chain, not just per-row hash).** Beyond the standalone `result_hash` (a fingerprint of a *result body* that is **not** stored, FR-024), each row carries a monotonic per-tenant **`seq`** and links the previous row via **`prev_hash`** → **`entry_hash` = H(prev_hash ‖ canonical(entry) ‖ seq)**, so a deleted/edited/reordered row breaks the chain (a `seq` gap or hash mismatch). The per-tenant chain head is authoritative Redis state at **`audit:head:{kind}:{tenant}`**, advanced atomically on each append. This upgrades the per-row fingerprint into a verifiable history **without changing what content is stored** (still refs + hashes only, never bodies). `sandbox_run` (below) is the same shape with the same chain columns.
- **Reusability seam**: `audit_log` + `agent_audit_log` (+ `sandbox_run`) are the durable backing of the domain-agnostic `Recorder`/`Sink`/`HashChain`/`AuditQuery` ports — `workspace_id` is the opaque `Tenant`, `actor_type`/`agent_role` fold into one `Actor{Kind,ID,Role}` (a member action is `Kind:"user"`, an AI tool call `Kind:"agent"`, a sandbox run `Kind:"sandbox"`), `action`/`tool_called` become a registered `Action`, and the `Sink` is `PostgresSink` over these partitions. Adding an audited surface is a new `Actor.Kind`/`Action` value, **not** a new table; a second durable target (SIEM/Kafka/S3) is a registered `Sink`, not an edit to the write path. See [contracts/audit-ports.md](./contracts/audit-ports.md).

### Sandbox Run Record (P)
Per-execution record for the sandbox tier (crawl / convert / code-gen microVMs) — the sandbox analogue of `agent_audit_log`, folded behind the same audit `Recorder` (`Actor{Kind:"sandbox"}`). See [contracts/sandbox-runtime.md](./contracts/sandbox-runtime.md) and [contracts/audit-ports.md](./contracts/audit-ports.md).
- `sandbox_run` (P): `id`, `workspace_id`, `user_id`, `template` (`tmpl-crawl`|`tmpl-convert`|`tmpl-coderun`), `feature` (`ingest.crawl`|`ingest.convert`|`agent.run_script`), `exit_code` INT, `vcpu_seconds` INT, `wall_ms` INT, `egress_bytes` BIGINT, `timed_out` BOOL, `result_hash` (tamper-evident result fingerprint), `trace_id`, `created_at`, **`seq` BIGINT + `prev_hash` + `entry_hash` (per-tenant tamper-evident chain link)**. Partitioned by `created_at`.
