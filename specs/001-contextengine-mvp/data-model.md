...
> **Owned by `intel-agent`.** The schema for `agent_policies` is defined in [the agent runtime's data model](https://github.com/truongpx396/intel-agent/blob/main/specs/001-agent-runtime/data-model.md) and shipped as its migration series; this repo applies it rather than re-specifying it. Re-declaring the columns here would create a second definition that drifts on the first upstream change.

This product still **applies** the table and reads it: it is the backing row for the `AgentManifest` the runtime loads per run. What belongs here is only the AISAT-specific binding — see [contracts/agent-integration.md](./contracts/agent-integration.md).
...
> **Owned by `intel-agent`.** The schema for `agent_run` is defined in [the agent runtime's data model](https://github.com/truongpx396/intel-agent/blob/main/specs/001-agent-runtime/data-model.md) and shipped as its migration series; this repo applies it rather than re-specifying it. Re-declaring the columns here would create a second definition that drifts on the first upstream change.
